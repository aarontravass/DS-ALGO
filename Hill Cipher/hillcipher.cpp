...
int main() {
	...
	cin>>s;
	vector<vector<int>> text(2,vector<int>(ceil(s.length()/2)));
	int c=0;
	for(int i=0;i<text[0].size();i++){
	    for(int j=0;j<2;j++){
	        text[j][i]=s[c]-'A';
	...
	for(int i=0;i<inv[0].size();i++) for(int j=0;j<2;j++) dcip+=char(decrypttext[j][i]+65);
	
	cout<<"Decryptd string is : "<<dcip<<endl;
